...
function markGeoColumns(schema, key_value_metadata) {
  ...
  const decodedColumns = (geo && ((_b = JSON.parse(geo)) == null ? void 0 : _b.columns)) ?? {};
  for (const [name, column] of Object.entries(decodedColumns)) {
    ...
    const id = ((_c = column.crs) == null ? void 0 : _c.id) ?? ((_e = (_d = column.crs) == null ? void 0 : _d.ids) == null ? void 0 : _e[0]);
...
async function asyncBufferFromUrl({ url, byteLength, requestInit, fetch: customFetch }) {
  ...
  const fetch = customFetch ?? globalThis.fetch;
  byteLength ?? (byteLength = await byteLengthFromUrl(url, requestInit, fetch));
  let buffer = void 0;
...
async function parquetReadColumn(options) {
  var _a;
  if (((_a = options.columns) == null ? void 0 : _a.length) !== 1) {
...
async function parquetReadRows(options) {
  ...
  options.metadata ?? (options.metadata = await parquetMetadataAsync(file, options));
