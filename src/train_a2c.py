"""A2C training loop. Episode-aligned: 1 rollout = 1 trading day.

Portfolio MDP (PROPOSAL.md Sec. 3-4): shared `RunningStd` reward normalizer
across episodes; metrics computed from `info["pnl_log"]` so they are invariant
to reward mode (r1/r2/r4). Eval/test envs use no normalization.

Reuses `DayCycleEnv`, `pooled_metrics`, `trade_pnls_from_session`,
`episode_max_dd_dollar`, `evaluate_policy_per_session`, `METRICS_HEADER`,
`METRICS_COLS`, `_row_from_metrics` from `src/train.py`.
"""
...
def train_a2c(cfg: dict) -> dict:
    ...
    agent = A2CAgent(
        obs_dim=obs_dim,
        n_actions=n_actions,
        hidden_sizes=list(a2c_cfg["hidden_sizes"]),
        lr=float(a2c_cfg["lr"]),
        gamma=float(a2c_cfg["gamma"]),
        gae_lambda=float(a2c_cfg["gae_lambda"]),
        value_coef=float(a2c_cfg["value_coef"]),
        entropy_coef=float(a2c_cfg["entropy_coef"]),
    ...
    rollout = Rollout()
    # Parallel per-bar collectors for portfolio metrics (NOT used by the optimizer).
        ...
        if terminated or truncated:
            ...
            if train_episodes_done % log_interval == 0:
                ...
                print(f"[ep {train_episodes_done:04d}] step={gs}  "
                      ...
                      f"H={agent.last_entropy:.3f} EV={agent.last_explained_var:+.2f}  "
    ...
    hparams = {
        ...
        "entropy_coef": float(a2c_cfg["entropy_coef"]),
        "value_coef": float(a2c_cfg["value_coef"]),
        "hidden_sizes": str(list(a2c_cfg["hidden_sizes"])),
        "best_metric": best_metric,
        "reward_mode": str(cfg["env"].get("reward", {}).get("mode", "r1")),
