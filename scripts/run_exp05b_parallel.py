"""Experiment 0.5b -- gamma=0.3 generalization to A2C and PPO (PROPOSAL.md Sec. 8.1).

Phase 1a (Exp 0.5a) established gamma=0.30 is optimal for DDQN. This phase tests
whether the finding is algorithm-agnostic (Zhang/Zohren/Roberts 2019 claim it
is -- they used gamma=0.3 for DQN, PG, A2C alike).

Sweep:  2 algos {A2C, PPO} x 3 seeds {42, 1337, 2026} x gamma=0.3 = 6 runs.
Reward: R4 (Exp 0 winner). Same per-reward lr as Exp 0 (config lr_per_reward).
Steps:  300k per run (~10 min/run parallel N=4 -> ~20 min total for 6 runs).

Outputs (under runs/exp05b/):
  - <algo>_s<seed>/                   per-run artifacts
  - exp05b_summary.csv                one row per run
  - exp05b_report.json                collapse rate + mean metrics per algo
"""
...
def _launch_worker(cfg: dict, algo: str, run_name: str, log_path: Path) -> subprocess.Popen:
    ...
    env["PYTHONIOENCODING"] = "utf-8"  # avoid cp874 errors on Thai Windows
    ...
    for entry in active:
        ...
        if bi_path.exists():
            try:
                ...
                print(f"  [WARN] {run_name}: best_info.json parse: {e}")
        if summary_path.exists():
            try:
                summary = json.loads(summary_path.read_text(encoding="utf-8"))
                test_m = summary.get("test_metrics", {})
            except Exception as e:
                print(f"  [WARN] {run_name}: summary.json parse: {e}")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Exp 0.5b -- gamma=0.3 on A2C/PPO")
    ...
    print(f"Experiment 0.5b -- gamma=0.3 generalization (A2C, PPO)")
    ...
    summary_csv = out_root / "exp05b_summary.csv"
    fieldnames = (
    ...
    report_path = out_root / "exp05b_report.json"
