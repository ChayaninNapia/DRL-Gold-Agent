"""Experiment 0.5d -- A2C/PPO with BC warm-start (PROPOSAL Sec. 8, post-Phase-1c).

Phase 1c (4-knob hyperparameter fix) was unable to escape policy collapse for
on-policy methods (H stayed at ln(3)=1.099, KL near 0, val_trades < 50 for most
seeds). Brainstorm consensus (2 of 3 sub-agents converged independently):
   the bottleneck is "rollout has no profitable trades -> no advantage signal
   -> no policy gradient -> stuck at uniform random". Fix = inject an EXTERNAL
   signal via behavior cloning on a daily hindsight expert.

iRDPG (Liu et al. AAAI 2020) ablation Table 2: BC on hindsight expert lifts
intraday futures Tr% from 6.96 to 28.34 (~4x). FinRL_Imitation_Learning repo
adopts the same pattern for PPO/A2C on equities.

Phase 1d setup:
  - Same 4-knob fix as Phase 1c (gae_lambda=1.0, value_coef=0.25,
    entropy_coef=0.05, ppo.n_epochs=4).
  - Plus BC: coef=1.0, anneal_steps=100k (1/3 of 300k training), lookahead=5
    bars (matches gamma=0.3 effective horizon), noise_threshold=0.0005.
  - 2 algos x 3 seeds = 6 runs at 300k steps each.

Outputs (under runs/exp05d/):
  - <algo>_s<seed>/                   per-run artifacts
  - exp05d_summary.csv                one row per run
  - exp05d_report.json                collapse rate + mean metrics per algo
"""
...
# Phase 1c 4-knob fix (kept; BC adds on top, not replaces).
A2C_OVERRIDES = {
    "gae_lambda": 1.0,
    "value_coef": 0.25,
    "entropy_coef": 0.05,
...
# Phase 1d new knob: BC warm-start. lr_bc is per-algo: the BC cross-entropy
# loss is ~scale 1.0, so it needs an R1-scale lr (a2c base 7e-4, ppo base 3e-4)
# to actually move the policy. lr anneals down to the small R4 RL lr by
# anneal_steps (lr schedule Option 1, see config.yaml bc.lr_bc comment).
BC_OVERRIDES = {
    "coef": 1.0,
...
# Per-algo lr during the BC phase (R1-scale; from the lr sweep that showed
# 1e-5 cannot move the policy under BC, 7e-4 converges well for A2C).
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Exp 0.5d -- A2C/PPO with BC warm-start")
    ...
    print(f"Experiment 0.5d -- A2C/PPO with BC warm-start (iRDPG-style)")
    ...
    summary_csv = out_root / "exp05d_summary.csv"
    fieldnames = (
    ...
    report_path = out_root / "exp05d_report.json"
